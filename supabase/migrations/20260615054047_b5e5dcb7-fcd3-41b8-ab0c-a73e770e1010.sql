...
CREATE OR REPLACE FUNCTION public.search_profiles(search_term TEXT, exclude_id UUID DEFAULT NULL)
RETURNS TABLE (id UUID, display_name TEXT, username TEXT, apelido TEXT, email TEXT, whatsapp TEXT, avatar_url TEXT, city TEXT)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN QUERY
  SELECT p.id, p.display_name, p.username, p.apelido, p.email, p.whatsapp, p.avatar_url, p.city
...
CREATE OR REPLACE FUNCTION public.send_profile_link_request(p_target_id UUID)
...
BEGIN
  ...
  END IF;
  SELECT id INTO v_existing_id FROM public.profile_links
    WHERE requester_id = v_requester_id AND target_id = p_target_id AND status = 'pending';
  IF v_existing_id IS NOT NULL THEN
    RETURN json_build_object('error', 'Pending request already exists');
  END IF;
  SELECT id INTO v_existing_id FROM public.profile_links
    WHERE requester_id = v_requester_id AND target_id = p_target_id AND status = 'accepted';
  IF v_existing_id IS NOT NULL THEN
  ...
  SELECT p_target_id, 'profile_link_request', 'Solicitação de vínculo de perfil',
    p.display_name || ' quer vincular o perfil a você.', '/perfil'
  FROM public.profiles WHERE id = v_requester_id;
  RETURN json_build_object('success', true, 'id', v_existing_id);
END; $$;

CREATE OR REPLACE FUNCTION public.respond_to_profile_link_request(p_link_id UUID, p_status public.link_status)
RETURNS JSON LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
...
BEGIN
  SELECT * INTO v_link FROM public.profile_links
  ...
  END IF;
  UPDATE public.profile_links SET status = p_status WHERE id = p_link_id;
  IF p_status = 'accepted' THEN
    SELECT requester_id INTO v_requester_id FROM public.profile_links WHERE id = p_link_id;
    SELECT display_name INTO v_requester_name FROM public.profiles WHERE id = v_user_id;
    INSERT INTO public.notifications (user_id, kind, title, body, link_url)
    VALUES (v_requester_id, 'profile_link_accepted', 'Vínculo de perfil aceito',
      v_requester_name || ' aceitou seu pedido de vínculo.', '/perfil');
  END IF;
  RETURN json_build_object('success', true);
END; $$;

CREATE OR REPLACE FUNCTION public.list_my_profile_links()
RETURNS TABLE (id UUID, linked_user_id UUID, linked_user_name TEXT, linked_user_apelido TEXT,
  linked_user_username TEXT, linked_user_avatar_url TEXT, linked_user_city TEXT,
  status public.link_status, created_at TIMESTAMPTZ, is_requester BOOLEAN)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN QUERY
  SELECT pl.id,
    CASE WHEN pl.requester_id = auth.uid() THEN pl.target_id ELSE pl.requester_id END,
    CASE WHEN pl.requester_id = auth.uid() THEN p2.display_name ELSE p1.display_name END,
    CASE WHEN pl.requester_id = auth.uid() THEN p2.apelido ELSE p1.apelido END,
    CASE WHEN pl.requester_id = auth.uid() THEN p2.username ELSE p1.username END,
    CASE WHEN pl.requester_id = auth.uid() THEN p2.avatar_url ELSE p1.avatar_url END,
    CASE WHEN pl.requester_id = auth.uid() THEN p2.city ELSE p1.city END,
    pl.status, pl.created_at, pl.requester_id = auth.uid()
  ...
  WHERE (pl.requester_id = auth.uid() OR pl.target_id = auth.uid())
    AND pl.status = 'accepted'
  ORDER BY pl.created_at DESC;
END; $$;

CREATE OR REPLACE FUNCTION public.list_pending_link_requests()
RETURNS TABLE (id UUID, requester_id UUID, requester_name TEXT, requester_apelido TEXT,
  requester_username TEXT, requester_avatar_url TEXT, requester_city TEXT, created_at TIMESTAMPTZ)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN QUERY
  SELECT pl.id, pl.requester_id, p.display_name, p.apelido, p.username, p.avatar_url, p.city, pl.created_at
  FROM public.profile_links pl
  JOIN public.profiles p ON pl.requester_id = p.id
  WHERE pl.target_id = auth.uid() AND pl.status = 'pending'
  ORDER BY pl.created_at DESC;
END; $$;

REVOKE EXECUTE ON FUNCTION public.search_profiles(TEXT, UUID) FROM anon;
REVOKE EXECUTE ON FUNCTION public.send_profile_link_request(UUID) FROM anon;
REVOKE EXECUTE ON FUNCTION public.respond_to_profile_link_request(UUID, link_status) FROM anon;
REVOKE EXECUTE ON FUNCTION public.list_my_profile_links() FROM anon;
REVOKE EXECUTE ON FUNCTION public.list_pending_link_requests() FROM anon;

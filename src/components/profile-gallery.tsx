...
export function ProfileGallery() {
  ...
  const photosQ = useQuery({ queryKey: ["gallery_photos"], queryFn: fetchPhotos, enabled: authChecked });
  ...
  return (
    <Card className="p-5 shadow-card">
      ...
      {photosQ.isLoading ? (
        ...
        <button onClick={() => setOpen(true)} className="w-full flex flex-col items-center justify-center gap-2 py-10 rounded-lg border-2 border-dashed border-border text-muted-foreground text-sm hover:bg-secondary/40 transition">
          <ImagePlus className="size-8 opacity-60"/>
          Clique para adicionar suas fotos
        </button>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {photos.map(p => <PhotoCard key={p.id} photo={p} userId={userId} onOpen={() => setViewer(p)} onLike={(liked) => likeMut.mutate({ photoId: p.id, liked })}/>)}
        </div>
      )}

      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setFile(null); setDescription(""); } }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Nova foto</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="g-file">Imagem</Label>
              <input id="g-file" type="file" accept="image/*" onChange={e => setFile(e.target.files?.[0] ?? null)} className="block w-full text-sm file:mr-3 file:py-2 file:px-3 file:rounded-md file:border-0 file:bg-secondary file:text-foreground file:text-xs file:font-semibold"/>
            </div>
            {preview && <img src={preview} alt="preview" className="w-full max-h-64 object-cover rounded-lg"/>}
            <div className="space-y-1.5">
              <Label htmlFor="g-desc">Descrição</Label>
              <Textarea id="g-desc" rows={3} value={description} onChange={e => setDescription(e.target.value)} placeholder="Conte sobre essa foto..."/>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={!!viewer} onOpenChange={(o) => !o && setViewer(null)}>
        <DialogContent className="max-w-2xl">
          {viewer && (
            <div className="space-y-3">
              <SignedImg path={viewer.image_url} className="w-full max-h-[60vh] object-contain rounded"/>
              {viewer.description && <p className="text-sm">{viewer.description}</p>}
              <div className="flex items-center justify-between">
                <div className="text-xs text-muted-foreground">{new Date(viewer.created_at).toLocaleString("pt-BR")}</div>
                {viewer.user_id === userId && (
                  <Button size="sm" variant="ghost" onClick={() => deleteMut.mutate(viewer)} className="text-destructive hover:text-destructive">
...
function PhotoCard({ photo, userId, onOpen, onLike }: { photo: PhotoRow; userId: string; onOpen: () => void; onLike: (liked: boolean) => void }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-secondary/40 border border-border/60 flex flex-col">
      <button type="button" onClick={onOpen} className="aspect-square overflow-hidden group">
        <SignedImg path={photo.image_url} className="w-full h-full object-cover transition group-hover:scale-105"/>
      </button>
      <div className="p-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn("inline-flex items-center gap-1 text-xs font-semibold rounded-full px-2.5 py-1 transition",
          ...
          <Avatar className="size-6 ml-auto"><AvatarFallback className="text-[10px]">{photo.user_id === userId ? "EU" : "?"}</AvatarFallback></Avatar>
        </div>
      </div>
    </div>
  );

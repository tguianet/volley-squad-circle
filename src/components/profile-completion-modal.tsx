...
async function fetchMyProfile(): Promise<ProfileRow | null> {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select(
      "id, display_name, apelido, city, state, whatsapp, posicao_principal, level, mao_dominante, genero, avatar_url, data_nascimento, altura, observacoes, bio, instagram, status",
    )
    .eq("id", u.user.id)
    .maybeSingle();
...
export function ProfileCompletionModal() {
  ...
  useEffect(() => {
    ...
    if (!v || v.startsWith("http") || v.startsWith("blob:")) {
  ...
  const avatarSrc =
    previewBlob ?? signedPreview ?? (form.avatar_url?.startsWith("http") ? form.avatar_url : null);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      qc.setQueryData(["my-profile"], (prev: ProfileRow | undefined) =>
      ...
      toast.error(getErrorMessage(e, "Erro ao salvar perfil"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open}>
      ...
      >
        <DialogHeader>
          <DialogTitle>
            Complete seu perfil <span className="text-primary">PlayBeach</span>
          </DialogTitle>
          <DialogDescription>
            Para entrar no ranking PlayBeach e desafiar outros jogadores, preencha as informações
            abaixo.
            {profile?.display_name && (
              <span className="block mt-1 text-xs">
                Olá, <b>{profile.display_name}</b> — seu nome do Google será usado no ranking.
              </span>
            )}
          </DialogDescription>
        </DialogHeader>
            ...
            <Field label="Foto de perfil">
              <div className="flex items-center gap-3">
                <Avatar className="size-16">
                  ...
                  <AvatarFallback>
                    {(form.apelido || profile?.display_name || "?").slice(0, 2).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <input
                  ref={fileRef}

...
function ProfilePage() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      qc.setQueryData(["my-profile"], (prev: MyProfile | undefined) =>
        ...
        <Card className="overflow-hidden shadow-card">
          <div className="relative">
            ...
            <div className="absolute -bottom-12 left-6 z-10">
              <ProfileAvatar
                ...
                className="size-24 ring-4 ring-background shadow-glow"
              />
            </div>
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button size="sm" variant="secondary" className="absolute top-3 right-3 z-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Nome vindo da sua conta Google.
                    </p>
                  </div>
          ...
          <div className="pt-16 px-6 pb-6">
            <div className="flex flex-wrap items-end gap-3">
              ...
              {profile.level && (
                <Badge className="gradient-beach text-white border-0 ml-auto">
                  {profile.level}
                </Badge>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 text-xs">
              ...
              <Info
                icon={Ruler}
                label="Altura"
                value={profile.altura ? `${profile.altura} m` : "—"}
              />
              <Info icon={Hand} label="Mão" value={profile.mao_dominante || "—"} />
              <Info icon={Target} label="Posição" value={profile.posicao_principal || "—"} />
              {profile.whatsapp && (
                <Info icon={MessageCircle} label="WhatsApp" value={profile.whatsapp} />
              )}
              {profile.instagram && (
                <Info icon={Instagram} label="Instagram" value={profile.instagram} />
              )}
            </div>
          </div>
        </Card>
...
function Stat({
  ...
  return (
    <Card className={`p-4 shadow-card ${accent ? "gradient-beach text-white" : ""}`}>
      ...
      {sub && (
        <div className={`text-[10px] mt-1 ${accent ? "opacity-80" : "text-muted-foreground"}`}>
          {sub}
        </div>
      )}
    </Card>
      ...
      {received.length > 0 && (
        <div className="mb-4 space-y-2">
          ...
          {received.map((r) => (
            <div
              key={r.id}
              className="flex items-center gap-2 p-2.5 rounded-md border bg-primary/5"
            >
            ...
            return (
              <div key={t.id} className="rounded-lg border p-3 space-y-3">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Tem certeza que deseja deletar este time? Essa ação removerá todos os
                              membros e não poderá ser desfeita.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancelar</AlertDialogCancel>
                ...
                <div className="space-y-1.5">
                  ...
                  <div
                    className={`flex items-center gap-2 p-2 rounded-md ${isCaptain ? "bg-primary/10" : "bg-secondary/40"}`}
                  >
                    {isCaptain ? (
                    ...
                    <div className="flex-1 text-sm">
                      {isCaptain ? "Você é o capitão" : "Você é membro"}
                    </div>
                  </div>
                  {isCaptain &&
  ...
  return (
    <Card className="p-5 shadow-card">
      <div className="flex items-center justify-between mb-3">
        ...
        <Link to="/h2h" className="text-xs text-primary font-semibold">
          Comparar H2H →
        </Link>
      </div>
      {data.length === 0 ? (
        ...
        <ul className="space-y-2">
          {data.map((m) => (
            <li
              key={m.id}
              className="flex items-center justify-between p-2.5 rounded-lg bg-secondary/60 text-sm"
            >
              <div className="min-w-0">

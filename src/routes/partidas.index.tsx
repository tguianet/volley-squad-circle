...
const MOD_LABEL: Record<string, string> = {
  beach_volley: "Vôlei de praia", indoor_volley: "Vôlei indoor", futevolei: "Futevôlei",
};
...
function MatchesPage() {
  ...
  const { data: matches = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("matches")
        .select("id, title, modality, match_type, date, start_time, end_time, max_players, status, arena:arena_id(name, city), players:match_players(player_id, status)")
        .in("status", ["open", "full"])
  ...
  async function join(matchId: string) {
    ...
    const { error } = await supabase.from("match_players").insert({
      match_id: matchId, player_id: user.id, status: "confirmed" as any,
    });
  ...
  async function leave(matchId: string) {
    ...
    const { error } = await supabase.from("match_players")
      .delete().eq("match_id", matchId).eq("player_id", user.id);
    if (error) { toast.error(error.message); return; }
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        {!isLoading && matches.length === 0 && (
          <Card className="p-10 text-center shadow-card">
            ...
            <p className="text-sm text-muted-foreground">Crie a primeira partida amistosa e convoque a galera.</p>
          </Card>
        )}

        <div className="space-y-3">
          {matches.map((m: any) => {
            const confirmed = (m.players ?? []).filter((p: any) => p.status === "confirmed").length;
            const isIn = (m.players ?? []).some((p: any) => p.player_id === user?.id);
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground flex flex-wrap gap-2 mt-1">
                      ...
                      <span className="flex items-center gap-1"><Clock className="size-3"/>{m.start_time?.slice(0,5)}{m.end_time ? ` – ${m.end_time.slice(0,5)}` : ""}</span>
                    </div>
                  </div>
                  <Badge variant={isFull ? "secondary" : "default"}>{isFull ? "Lotada" : "Aberta"}</Badge>
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground mb-3">
                  <Badge variant="outline">{MOD_LABEL[m.modality] ?? m.modality}</Badge>
                  <Badge variant="outline">{m.match_type}</Badge>
                  <span className="ml-auto">{confirmed}/{m.max_players} vagas</span>
                </div>
                {isIn ? (
                  <Button variant="outline" className="w-full" onClick={() => leave(m.id)}>Cancelar inscrição</Button>
                ) : (
                  <Button className="w-full gradient-beach text-white border-0" disabled={isFull} onClick={() => join(m.id)}>
